...
from mlp_functions import get_model
...
data = pd.read_csv("SMART.csv", header=0)
data_outliers = pd.read_csv("SMART.csv", header=0)
...
df.to_json("json\\noisy_results.json", orient='records')
...
df.to_json("json\\outliers_results.json", orient='records')
...
df = pd.DataFrame({"preds": mlp_preds_noisy, "repr": mlp_repr_noisy, "label": test_label})
df.to_json("json\\clean_results.json", orient='records')
