...
from SAS_SSD_mlp_functions import kde2D_visualisation


# Read training and testing data
df_train = pd.read_json("json\\SAS-SSD_train_results.json")
df_outliers = pd.read_json("json\\SAS-SSD_outliers_results.json")
df_noisy = pd.read_json("json\\SAS-SSD_noisy_results.json")
df_clean = pd.read_json("json\\SAS-SSD_clean_results.json")
...
title = 'SAS-SSD Outliers : ' + r'$\epsilon$' + ' = 0.1'
plt.title(title)
# alter next line command with clean/outlier/noisy in the file name
plt.savefig('SAS-SSD_density_outlier_eng.eps', format='eps')
plt.close()
